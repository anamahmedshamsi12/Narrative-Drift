...
| **Par level** | The stock threshold a pharmacy reorders at — not a regulatory term, just standard retail inventory practice. The Inventory view's `PAR_LEVELS` are invented reference numbers for this demo, not real wholesale ordering thresholds. |
| **OpenFDA** | The FDA's public, no-API-key-required API exposing structured drug label data (`drug/label.json`). It does **not** expose DEA schedule or a structured drug-drug interaction matrix — both the drug tooltip's schedule badge and the Interactions view's severity assessment are clear about which parts come from this API versus from local knowledge or Claude's own reasoning, rather than presenting everything as one authoritative source. |
